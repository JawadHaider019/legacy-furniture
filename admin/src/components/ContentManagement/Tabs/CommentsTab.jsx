  ...
  const fetchComments = async () => {
    try {
      ...
      const formattedComments = commentsData.map((comment) => ({
        ...
        targetType: comment.targetType || "product",
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex border-b border-gray-200 mb-4 sm:mb-6 overflow-x-auto">
        <button
          className={`flex-shrink-0 px-3 sm:px-4 py-2 sm:py-3 font-medium transition-colors whitespace-nowrap text-sm sm:text-base ${filter === 'all' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-gray-700'
            }`}
        ...
        <button
          className={`flex-shrink-0 px-3 sm:px-4 py-2 sm:py-3 font-medium transition-colors whitespace-nowrap text-sm sm:text-base ${filter === 'unread' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-gray-700'
            }`}
        ...
        <button
          className={`flex-shrink-0 px-3 sm:px-4 py-2 sm:py-3 font-medium transition-colors whitespace-nowrap text-sm sm:text-base ${filter === 'replied' ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-gray-700'
            }`}
                        ...
                        <td className="px-4 sm:px-6 py-4">
                          ...
                          <div className="flex items-center space-x-3 mt-2">
                            <button
                              ...
                              className={`flex items-center space-x-1 text-xs transition-colors ${hasLiked
                                ? 'text-green-600'
                            ...
                            <button
                              ...
                              className={`flex items-center space-x-1 text-xs transition-colors ${hasDisliked
                                ? 'text-red-600'
                  ...
                  <div key={comment._id} className={`p-4 ${!comment.isRead ? 'bg-blue-50 border-l-4 border-l-black' : ''}`}>
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center space-x-2">
                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${comment.isRead
                          ? 'bg-green-100 text-green-800'
                          : 'bg-yellow-100 text-yellow-800'
                          }`}>
                          <FontAwesomeIcon
                    ...
                    <div className="flex items-center space-x-4 mb-3">
                      <button
                        onClick={() => likeComment(comment._id)}
                        className={`flex items-center space-x-1 text-sm transition-colors ${hasLiked
                          ? 'text-green-600'
                          : 'text-gray-600 hover:text-green-600'
                          }`}
                      >
                      ...
                      <button
                        onClick={() => dislikeComment(comment._id)}
                        className={`flex items-center space-x-1 text-sm transition-colors ${hasDisliked
                          ? 'text-red-600'
                          : 'text-gray-600 hover:text-red-600'
                          }`}
                      >
              ...
              <div className="absolute bottom-2 sm:bottom-4 left-2 right-2 flex justify-center space-x-1 sm:space-x-2 overflow-x-auto py-1">
                {currentImages.map((image, index) => (
                  <img
                    ...
                    className={`w-8 h-8 sm:w-12 sm:h-12 object-cover rounded cursor-pointer border-2 flex-shrink-0 ${index === currentImageIndex ? 'border-white' : 'border-transparent'
